...
function action(mode, type, selection) {
	...
	if (cm.getJob() == 0) {
		...
		cm.sendOk("等级尚未达到10级,你还不能转职成为 #r弓箭手#k");
		cm.dispose();
	    }
	} else {
	...
	cm.sendOk("转职成功!");
	cm.worldMessage("‘转职快报’：恭喜玩家."+ cm.getChar().getName() +"  成功弓箭手一转,让我们热烈的祝福他/她吧！");
	cm.dispose();
    } else if (status == 11) {
	...
	cm.sendOk("转职成功!");
	cm.worldMessage("‘转职快报’：恭喜玩家."+ cm.getChar().getName() +"  成功弓箭手二转,让我们热烈的祝福他/她吧！");
	cm.dispose();
    }
}	
