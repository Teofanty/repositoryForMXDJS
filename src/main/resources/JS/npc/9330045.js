...
function action(mode, type, selection) {
	...
	if (sel == 0) {
	    ...
	    cm.sendYesNo("需要150000金币购买240个鱼饵。你想购买?");
	} else if (sel == 2) {
	    if (cm.haveItem(3011000)) {
	    ...
	    if (cm.canHold(2300001,240) && cm.haveItem(5350000,1)) {
		if (!cm.haveItem(2300001)) {
		    cm.gainItem(2300001, 240);
	...
	if (sel == 1) {
	    if (cm.canHold(2300000,260) && cm.getMeso() >= 150000) {
		if (!cm.haveItem(2300000)) {
		    cm.gainMeso(-150000);
		    cm.gainItem(2300000, 240);
		    cm.sendNext("快乐钓鱼~");
		} else {
		    cm.sendNext("你已经有了钓鱼的诱饵.");
		}
