...
function action(mode, type, selection) {
    if (cm.getMapId() == 926100600) {
		...
		if (em != null) {
			...
			cm.gainItem(4310030, 4);
	        cm.gainItem(4031456, 1);
			cm.gainExp(290000);//20W经验
    ...
    if (status == 0) {
		...
		if (selection == 0) {
			...
			if (cm.canHold(cm.isGMS() ? 1122118 : 1122010,1) && cm.haveItem(4001160,40) ) {
				cm.gainItem(1122118,1);
				cm.gainItem(4001160,-40);
				//cm.gainItem(4001159,-20);
			} else {
				cm.sendOk("你将需要40个#v4001160##k,得到爱的证明,以及确认你的背包是否有足够空间!");
			}
		}
    	cm.dispose();
    }
