		...
		if (status == 0) {
			if (battlers[0] == null) {
				if (cm.getPlayer().getLevel() < 30) {
					cm.sendOk("忍耐。回来的时候你是30级...");
					cm.dispose();
				} else if (cm.getPlayer().getBoxed().size() > 0) {
					cm.sendOk("您可以在框中怪物，请先说出来.");
					cm.dispose();
				} else {
					var selStr = "你想要什么?\r\n\r\n#b";
			...
			var selStr = "总胜: " + cm.getPlayer().getTotalWins() + ", 总损失: " + cm.getPlayer().getTotalLosses() + ", 当前WINS: " + cm.getPlayer().getIntNoRecord(122400) + "\r\n检查其中的统计数据?\r\n\r\n#b";
			...
			if (battlers[0] == null) {	
				...
				cm.sendOk("你已经获得你的第一个奴才。你可以低着头网吧训练还是让其他的奴才。您还可以使用@challenge<玩家名称>挑战别人比赛.");
				cm.dispose();
				return;
			}
			...
			if (cm.getPlayer().getBattle() != null) {
				...
				info += "#L4#更改 order of this monster.#l\r\n";
			...
			if (selection == 0) { //how i evolve
				...
				if (evo == 0) {
					...
					cm.sendNext("你仍然有很长的路要走，因为你必须平整更多一些.");
					cm.dispose();
				} else if (evo == 2) {
					...
					if (cm.haveItem(battlers[sel].getFamily().evoItem.id)) {
						...
						cm.sendNext(selStr + "您没有进化项目需要。 需要: #v" + battlers[sel].getFamily().evoItem.id + "##z" + battlers[sel].getFamily().evoItem.id + "#");
						cm.dispose();
					}
				}
				...
				cm.sendGetText("为你的怪物请输入新的名称。 （小型：2个字符，最多：20个字符)");
			} else if (selection == 4) {
				if (cm.getPlayer().getBattle() != null) {
					cm.dispose();	
					return;
				...
				var selStr = "更改 " + battlers[sel].getName() + " 与怪物?\r\n\r\n#b";
				for (var i = 0; i < battlers.length; i++) {
			...
			if (sec == 0) {
				...
				if (cm.getPlayer().removeBattler(sel)) {
					...
					cm.sendOk("你不能释放的最后一个怪物!");
				}
			} else if (sec == 3) {
				if (cm.getText().length() < 2 || cm.getText().length() > 20) {
					cm.sendOk(cm.getText() + " 不能被接受.");
				} else {
					cm.getPlayer().更改dBattler();
