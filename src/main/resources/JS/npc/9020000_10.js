...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) {
          ...
          text += "#L55##r兑换#k( 20个 #v4031560#+100抵用 兑换一个 #v4020009# )\r\n\r\n";
		   text += "#L56##r兑换#k( 4个 #v4031558#+100抵用 兑换一个 #v4020009# )\r\n\r\n";	
			...
			if (cm.getInventory(4).isFull(0)){
				...
				cm.dispose();
			} else if (!cm.haveItem(4031560,20)) {//判断物品
			    cm.sendOk("#b你没有20个#v4031560##z4031560#");
				cm.dispose();
			
			} else if(!cm.getPlayer().getCSPoints(1) >= 100){//判断抵用券
			cm.sendOk("#b你没有足够的抵用");
				cm.dispose();
			} else{
				cm.gainD(-100);//抵用券
				cm.gainItem(4031560, -20);//给物品
			...
			if (cm.getInventory(4).isFull(0)){
				cm.sendOk("#b请保证消耗栏位至少有1个空格,否则无法兑换");
				cm.dispose();
			} else if (!cm.haveItem(4031558,4)) {//判断物品
			    cm.sendOk("#b你没有4个#v4031558##z4031558#");
				cm.dispose();
