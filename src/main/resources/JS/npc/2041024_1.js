...
if(cm.getInventory(1).getItem(1).getUpgradeSlots()==0){
	...
	cm.dispose();		
		...
		if (!cm.haveItem(4020009, 50)) {
			cm.sendOk("#b你没有50个#v4020009##z4020009#.");
		    cm.dispose();
...
var sj = Math.floor(Math.random()*1.6);//几率比如1.6就是60%
...
var getOne1 = Math.floor(Math.random()*8);//几率随下面的属性
...
var txt =""
 if (getOne1==1){
	...
	citem.setStr(citem.getStr()+ 1); //力量
	citem.setInt(citem.getInt()+ 1); //敏捷
	citem.setLuk(citem.getLuk()+ 1); //智力
	citem.setDex(citem.getDex()+ 1); //运气
	citem.setHp(citem.getHp()+ 5); 
	citem.setMp(citem.getMp()+ 5); 
	citem.setMatk(citem.getMatk()+ 2); //魔法力
	citem.setWatk(citem.getWatk()+ 1); //物攻
	txt +="力量/敏捷/智力/运气+1  物攻+1 魔法力+2  HP/MP+5"; 	
} else if(getOne1==2){
	...
	cm.dispose();	 	 
         ...
         //cm.itemlaba("60%时间碎片发出一道闪光，在时间女神之戒上添加了某种神秘的力量+"+txt,citem);
		 cm.sendOk("#r#e60%时间碎片发出一道闪光，在时间女神之戒上添加了某种神秘的力量!#k \r\n"+txt); 
		 cm.dispose();
		}else {
		var itemId1 = cm.getInventory(1).getItem(1).getItemId();
		 var citem = cm.getInventory(1).getItem(1).copy();
		...
		MapleInventoryManipulator.removeFromSlot(cm.getC(),stype,1,1, false);
		 MapleInventoryManipulator.addFromDrop(cm.getC(), citem,false);
		cm.sendOk("#r#e60%时间碎片发出一道闪光，但是物品未发生任何变化！");
