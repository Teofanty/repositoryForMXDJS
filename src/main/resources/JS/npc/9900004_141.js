			...
			cm.dispose();
		       ...
		       /*if (!cm.haveItem(4251302, 1) || cm.getMeso() <= 5000000)  {
                ...
                cm.dispose();
				    ...
				    cm.dispose();
} else if((citem.getUpgradeSlots() +  citem.getLevel())>=100){
...
item.setFlag(1);
		 ...
		 MapleInventoryManipulator.addFromDrop(cm.getC(), item,false);
				...
				cm.laba(cm.getPlayer().getName() + "『强化公告』" + " : " + "在拍卖-强化成功！大家一起恭喜他（她）吧！",11);
				cm.dispose();
				 }else {
				cm.getPlayer().setBossLog('qianghua1');
				cm.sendOk("#r#e强化失败！,祝您游戏愉快!#k装备并未有任何变化!");	 
				cm.laba(cm.getPlayer().getName() + "『强化公告』" + " : " + "在拍卖-强化失败！大家一起恭喜他（她）吧！",11);
				cm.dispose();
				 }
