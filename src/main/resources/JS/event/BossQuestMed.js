...
function monsterSpawn(eim) { // Custom function
    ...
    switch (monsterid) {
        ...
        case 9300039: // Papa Pixie
            var modified = em.newMonsterStats();
            modified.setOExp(mob.getMobExp());
            modified.setOHp(mob.getMobMaxHp() * 1.4);
            modified.setOMp(mob.getMobMaxMp() * 1.4);
...
function allMonstersDead(eim) {
    ...
    eim.broadcastPlayerMsg(5, "你的团队赢了"+num+" 点！ 总共有 "+totalp+".");

    eim.saveBossQuest(num);
    
    if (mobnum < monster.length) {
        eim.broadcastPlayerMsg(6, "准备！ 下一个BOSS会瞥见眼睛!");
    } else {
        eim.saveBossQuest(2500);
        eim.broadcastPlayerMsg(5, "你的队伍打败了MED模式，并获得了额外的2500点!");
    }
