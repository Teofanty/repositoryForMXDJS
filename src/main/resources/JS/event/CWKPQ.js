...
function setup(eim, leaderid) {
    ...
    for (var i = 0; i < mapz.length; i++) {
        var map = eim.setInstanceMap(803010600 + mapz[i]);
        if (map != null) {
            map.resetFully();
            if (map.getId() == 803011000) {
            ...
            } else if (map.getId() == 803011110) { //warrior room, crimson guardians
            ...
            } else if (map.getId() == 803011140) { //bowman room, spawn master guardians
...
function playerEntry(eim, player) {
    eim.broadcastPlayerMsg(5, "[绯红远征队] " + player.getName() + " 已经进入地图！");
    var map = eim.getMapInstance(803010700 + (parseInt(em.getProperty("current_instance")) * 100));
...
function spawnGuardians(eim) {
    ...
    eim.broadcastPlayerMsg(5, "警卫已经发现了你.");
...
function changedMap(eim, player, mapid) {
    if (mapid < 803010700 || mapid > 803011300) {
...
function playerExit(eim, player) {
    eim.broadcastPlayerMsg(5, "[Expedition] " + player.getName() + " 离开了军械库.");
...
function end(eim) {
    eim.disposeIfPlayerBelow(100, 803011400);
    em.setProperty("state", "0");
    em.setProperty("leader", "true");
}
