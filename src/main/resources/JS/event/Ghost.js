...
function registerCarnivalParty(eim, carnivalParty) {
    if (eim.getProperty("red").equals("-1")) {
        ...
        // display message about recieving invites for next 3 分钟;
	//eim.restartEventTimer(180000);
        eim.schedule("end", 3 * 60 * 1000); // 3 分钟
    } else {
...
function getParty(eim, property) {
	...
	eim.broadcastPlayerMsg(5, "组队队长" + property + "没找到.");
	disposeAll(eim);
	return null;
    } else {
...
function onMapLoad(eim, chr) {
    ...
    if (!eim.getProperty("started").equals("true")) {
		...
		eim.broadcastPlayerMsg(6, "红队已经到达BOSS地图，将获得5％的提升.");
        	var iter = eim.getPlayers().iterator();
        	while (iter.hasNext()) {
		    var player = iter.next();
		...
		eim.broadcastPlayerMsg(6, "蓝队已经到达BOSS地图，将获得5％的提升.");
        	var iter = eim.getPlayers().iterator();
        	while (iter.hasNext()) {
		    var player = iter.next();
		    player.changeMap(chr.getMap(), chr.getMap().getPortal(player.getCarnivalParty().getTeam() == 0 ? "redTeam" : "blueTeam"));
