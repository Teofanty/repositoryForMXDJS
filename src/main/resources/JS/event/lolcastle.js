...
function announce() {
    ...
    if (i == 0) i = 5;
    em.getChannelServer().broadcastPacket(tools.MaplePacketCreator.serverNotice(6, "[事件] 审判现场将打开 " + i + " 分钟"));
    i--;
}
...
function mesoDistribution() {
	...
	if (eim.getPlayerCount() > 0) {
	    ...
	    map.broadcastMessage(tools.MaplePacketCreator.serverNotice(6, "[事件] " + winner.getName() + " 胜 " + meso + " 金币"));
...
function start() {
	...
	tools.MaplePacketCreator.serverNotice(6, "[事件]判决书场现已开放"));
    var iter = em.getInstances().iterator();
    while (iter.hasNext()) {
	var eim = iter.next();
	if (eim.getPlayerCount() > 0) {
...
function playerDead(eim, player) {
    ...
    player.getClient().getSession().write(tools.MaplePacketCreator.serverNotice(1, "你失去了与 " + eim.getKillCount(player) + " 点."));
}
...
function allMonstersDead(eim) {
	...
	winner.getClient().getSession().write(tools.MaplePacketCreator.serverNotice(1, "你赢了与 " + eim.getKillCount(winner) + " 点. 您将在2分钟内被传送了."));
