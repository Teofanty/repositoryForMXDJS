...
function registerCarnivalParty(eim, carnivalParty) {
    if (eim.getProperty("red").equals("-1")) {
        ...
        eim.schedule("end", 3 * 60 * 1000); // 3分钟执行
		eim.broadcastPlayerMsg(5, "接下來的三分您的队伍可以找寻其他人进行挑战");
    } else {
        eim.setProperty("blue", carnivalParty.getLeader().getId() + "");
        eim.schedule("start", 10000);//10秒执行
		eim.broadcastPlayerMsg(5, "正在检测是否异常.");//显示文字
    }
}
...
function playerRevive(eim, player) {
    //player.getCarnivalParty().useCP(player, 10);//死亡扣除CP
    var iter = eim.getPlayers().iterator();
    while (iter.hasNext()) {
	...
	player.addHP(500);//死后恢复的血量
	player.addMP(500);//死后恢复的蓝
