...
function playerDisconnected(eim, player) {
    ...
    eim.broadcastPlayerMsg(5, player.getName() + " 离开了副本");
	player.saveToDB(false, false);
    player.getPlayerStorage().deregisterPlayer(player);
	disposeAll(eim);
    return 0;
}
...
function monsterValue(eim, mobId) {
    if (mobId == 9300049 && em.getProperty("stage7").equals("0")) { //dark nep
        eim.broadcastPlayerMsg(6, "精灵爸爸已经衍生.");
        ...
        if (st < 14) {
            eim.broadcastPlayerMsg(6, "怪物已经在地图上的某处衍生.");
